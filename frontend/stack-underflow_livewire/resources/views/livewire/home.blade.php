    ...
    <div class="bg-white rounded-lg shadow mb-6 p-4">
        <div class="flex flex-wrap gap-4 items-center justify-between">
            <div class="flex gap-2">
                <select wire:model="sortBy" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand-orange focus:border-transparent">
                ...
                <div class="flex">
                    ...
                    <div class="flex-shrink-0 flex space-x-4 mr-4">
                        <div class="question-stat">
                            <span class="question-stat-value text-gray-700">{{ $question->votes }}</span>
                            <span class="text-xs text-gray-500">votes</span>
                        </div>
                        <div class="question-stat {{ $question->isAnswered ? 'text-green-600 border border-green-500 rounded px-2' : '' }}">
                            <span class="question-stat-value">{{ $question->answers }}</span>
                            <span class="text-xs text-gray-500">answers</span>
                        </div>
                        <div class="question-stat">
                            <span class="question-stat-value text-gray-500">{{ $question->views }}</span>
                        ...
                        <div class="flex flex-wrap items-center justify-between">
                            ...
                            <div class="flex items-center text-sm text-gray-500">
                                @if($question->author)
                                    ...
                                    <span class="text-gray-400">asked {{ $question->createdAt ? \Carbon\Carbon::parse($question->createdAt)->diffForHumans() : '' }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
